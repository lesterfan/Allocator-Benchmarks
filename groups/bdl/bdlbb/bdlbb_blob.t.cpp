...
class TestBlobBufferFactory : public bdlbb::BlobBufferFactory
{
    ...
    public:
    // CREATORS
    explicit TestBlobBufferFactory(bsl::size_t       currentBufferSize = 4,
                                   bool              growFlag = true);
...
//..
    ...
    void SimpleBlobBufferFactory::allocate(bdlbb::BlobBuffer *buffer)
    {
        ...
        bsl::shared_ptr<char> shptr(
                                (char *) myAllocator->allocate(d_bufferSize),
                                myAllocator);
...
//..
    int timestampMessage(bdlbb::Blob *blob, bslma::Allocator *allocator)
    {
        ...
        // Changed: SimpleBlobBufferFactory fa(128, allocator);
        SimpleBlobBufferFactory& fa = *new(*allocator) SimpleBlobBufferFactory(128); 
        bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp0(&fa, allocator);
...
//..

    {
        ...
        // Changed: bdlbb::Blob blob(&ta);
       	bdlbb::Blob& blob = *new(ta) bdlbb::Blob(); 
        bslma::RawDeleterProctor<bdlbb::Blob, bslma::Allocator> dp1(&blob, &ta);
...
//          bdlbb::PooledBlobBufferFactory factory(1024);
            // Changed: SimpleBlobBufferFactory       factory(1024, allocator);
        	SimpleBlobBufferFactory& factory = *new(ta) SimpleBlobBufferFactory(1024); 
            bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dpA(&factory, &ta);
        ...
        // Changed: SimpleBlobBufferFactory sbbf(1024, &ta);
        SimpleBlobBufferFactory& sbbf = *new(ta) SimpleBlobBufferFactory(1024); 
        bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp2(&sbbf, &ta);

        {
            // Changed: bdlbb::Blob blob(&sbbf, &ta);
        	bdlbb::Blob& blob = *new(ta) bdlbb::Blob(&sbbf); 
            bslma::RawDeleterProctor<bdlbb::Blob, bslma::Allocator> dp3(&blob, &ta);
...

        {
            // Changed: bdlbb::Blob blob(&sbbf, &ta);
        	bdlbb::Blob& blob = *new(ta) bdlbb::Blob(&sbbf); 
            bslma::RawDeleterProctor<bdlbb::Blob, bslma::Allocator> dp4(&blob, &ta);
...

        {
            // Changed: bdlbb::Blob blob(&sbbf, &ta);
        	bdlbb::Blob& blob = *new(ta) bdlbb::Blob(&sbbf); 
            bslma::RawDeleterProctor<bdlbb::Blob, bslma::Allocator> dp5(&blob, &ta);
      ...
      case 12: {
        ...
        BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
        {
            ...
            // Changed: SimpleBlobBufferFactory fa(BUFFER_SIZE, &ta); Lester
            // Changed: Obj mX(&fa, &ta);   const Obj& X = mX;
        	SimpleBlobBufferFactory& fa = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE); 
            bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp6(&fa, &ta);
        	Obj& mX = *new(ta) Obj(&fa); 
            bslma::RawDeleterProctor<Obj, bslma::Allocator> dp7(&mX, &ta); const Obj& X = mX;
...
//      bslma::TestAllocator& testAllocator = ta;
        ...
        for (int dataLength1 = 0;
            ...
            // Changed: SimpleBlobBufferFactory fa1(BUFFER_SIZE1, &ta);
            // Changed: Obj mX(&fa1, &ta);  const Obj& X = mX;
        	SimpleBlobBufferFactory& fa1 = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE1); 
            bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp8(&fa1, &ta);
        	Obj& mX = *new(ta) Obj(&fa1); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp9(&mX, &ta); const Obj& X = mX;

            mX.setLength(TOTAL_SIZE1);  // set up capacity
            mX.setLength(DATA_LENGTH1);

            // I am not changing bsl::string for now!
        ...
        for (int dataLength2 = 0;
            ...
            // Changed: SimpleBlobBufferFactory fa2(BUFFER_SIZE2, &ta);
            // Changed: Obj mZ1(&fa2, &ta);  const Obj& Z1 = mZ1;
            // Changed: Obj mZ2(&fa2, &ta);  const Obj& Z2 = mZ2;
        	SimpleBlobBufferFactory& fa2 = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE2); 
            bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp10(&fa2, &ta);
        	Obj& mZ1 = *new(ta) Obj(&fa2); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp11(&mZ1, &ta); const Obj& Z1 = mZ1;
        	Obj& mZ2 = *new(ta) Obj(&fa2); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp12(&mZ2, &ta); const Obj& Z2 = mZ2;
            ...
            // Changed: Obj mY1(X, &fa1, &ta); const Obj& Y1 = mY1;
            // Changed: Obj mY2(X, &fa1, &ta); const Obj& Y2 = mY2;
        	Obj& mY1 = *new(ta) Obj(X, &fa1); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp13(&mY1, &ta); const Obj& Y1 = mY1;
        	Obj& mY2 = *new(ta) Obj(X, &fa1); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp14(&mY2, &ta); const Obj& Y2 = mY2;
...
//          const int TOTAL_TOTAL2 = Z2.totalSize() + Y2.totalSize();
      ...
      case 10: {
             ...
             ++dataLength)
        {
            ...
            BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
            {
                ...
                typedef bdlbb::Blob Obj;
                // Changed: SimpleBlobBufferFactory fa(BUFFER_SIZE, &ta);
                // Changed: Obj mX(&fa, &ta);   const Obj& X = mX;
                SimpleBlobBufferFactory& fa = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE); 
                bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dpB(&fa, &ta);
        		Obj& mX = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp15(&mX, &ta); const Obj& X = mX;
      ...
      case 9: {
        ...
        for (int numBuffers = 0; numBuffers < 10; ++numBuffers) {
            ...
            // Changed: Obj mX(&fa, &ta);  const Obj& X = mX;
        	SimpleBlobBufferFactory& fa = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE); 
            bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp16(&fa, &ta);
        	Obj& mX = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp17(&mX, &ta); const Obj& X = mX;
            ...
            // Changed: Obj mY(X, &fa, &ta); const Obj& Y = mY;
        	Obj& mY = *new(ta) Obj(X, &fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp18(&mY, &ta); const Obj& Y = mY;

            ASSERT(X                         == Y );
            ASSERT(NUM_BUFFERS + 1           == Y.numBuffers());
            ASSERT(NUM_BUFFERS * BUFFER_SIZE == Y.length());

            // Changed: Obj mZ(&fa, &ta); const Obj& Z = mZ;
        	Obj& mZ = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp19(&mZ, &ta); const Obj& Z = mZ;
      ...
      case 8: {
        ...
        for (int prependSz   = 1; prependSz   <= 5; ++prependSz)
        {
            ...
            BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
            {
                ...
                // Changed: Obj mX(&fa, &ta);   const Obj& X = mX;
        		SimpleBlobBufferFactory& fa = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE); 
                bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp20(&fa, &ta);
        		Obj& mX = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp21(&mX, &ta); const Obj& X = mX;
                ...
                // Changed: SimpleBlobBufferFactory ifa(PREPEND_BUFFER_SIZE, &ta);
        		SimpleBlobBufferFactory& ifa = *new(ta) SimpleBlobBufferFactory(PREPEND_BUFFER_SIZE); 
                bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp22(&ifa, &ta);
        ...
        for (int appendSz   = 1; appendSz   <= 5; ++appendSz)
        {
            ...
            BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
            {
            	...
            	// Changed: Obj mX(&fa, &ta);   const Obj& X = mX;
        		SimpleBlobBufferFactory& fa = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE); 
                bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp23(&fa, &ta);
        		Obj& mX = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp24(&mX, &ta); const Obj& X = mX;
                ...
                // Changed: SimpleBlobBufferFactory ifa(APPEND_BUFFER_SIZE, &ta);
        		SimpleBlobBufferFactory& ifa = *new(ta) SimpleBlobBufferFactory(APPEND_BUFFER_SIZE); 
                bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp25(&ifa, &ta);
      ...
      case 7: {
        ...
        for (int removePos  = 0; removePos < numBuffers; ++removePos)
        {
            ...
            BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
            {
                ...
                // Changed: Obj mX(&fa, &ta);   const Obj& X = mX;
        		SimpleBlobBufferFactory& fa = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE); 
                bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp26(&fa, &ta);
        		Obj& mX = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp27(&mX, &ta); const Obj& X = mX;
             ...
             ++trimBufferIdx)
        {
            ...
            BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
            {
                ...
                // Changed: Obj mX(&fa, &ta);   const Obj& X = mX;
        		SimpleBlobBufferFactory& fa = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE); 
                bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp28(&fa, &ta);
        		Obj& mX = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp29(&mX, &ta); const Obj& X = mX;
        ...
        for (int removePos  = 0; removePos < numBuffers; ++removePos)
        {
            ...
            BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
            {
        		...
        		SimpleBlobBufferFactory& fa = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE); 
                bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp30(&fa, &ta);
        		Obj& mX = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp31(&mX, &ta); const Obj& X = mX;
             ...
             dataLength <= bufferSize * numBuffers; ++dataLength)
        {
            ...
            BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
            {
                ...
                // Changed: Obj mX(&fa, &ta);   const Obj& X = mX;
        		SimpleBlobBufferFactory& fa = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE); 
                bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp32(&fa, &ta);
        		Obj& mX = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp33(&mX, &ta); const Obj& X = mX;
      ...
      case 6: {
        ...
        for (int appendSz   = 0; appendSz   <= 5; ++appendSz)
        {
            ...
            BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
            {
                ...
                // Changed: Obj mX(&fa, &ta);   const Obj& X = mX;
        		SimpleBlobBufferFactory& fa = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE); 
                bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp34(&fa, &ta);
        		Obj& mX = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp35(&mX, &ta); const Obj& X = mX;
                ...
                // Changed: SimpleBlobBufferFactory ifa(APPEND_BUFFER_SIZE, &ta);
        		SimpleBlobBufferFactory& ifa = *new(ta) SimpleBlobBufferFactory(APPEND_BUFFER_SIZE); 
                bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp36(&ifa, &ta);
      ...
      case 5: {
        ...
        for (int insertSz   = 0; insertSz   <= 5; ++insertSz)
        {
            ...
            BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
            {
                ...
                // Changed: Obj mX(&fa, &ta);   const Obj& X = mX;
        		SimpleBlobBufferFactory& fa = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE); 
                bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp37(&fa, &ta);
        		Obj& mX = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp38(&mX, &ta); const Obj& X = mX;
                ...
                // Changed: SimpleBlobBufferFactory ifa(INSERT_BUFFER_SIZE, &ta);
        		SimpleBlobBufferFactory& ifa = *new(ta) SimpleBlobBufferFactory(INSERT_BUFFER_SIZE); 
                bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp39(&ifa, &ta);
      ...
      case 4: {
                                                  ...
                                                  ++dataLength)
        {
            ...
            bslma::DefaultAllocatorGuard guard(&ta);
            {
                ...
                // Changed: Obj mX(&fa, &ta);   const Obj& X = mX;
        		    SimpleBlobBufferFactory& fa = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE); 
                    bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp40(&fa, &ta);
        		    Obj& mX = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp41(&mX, &ta); const Obj& X = mX;
      ...
      case 3: {
        ...
        for (int dataLength = 0; dataLength <= 5 * bufferSize; ++dataLength)
        {
            ...
            BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
            {
                ...
                // Changed: Obj mX(&fa, &ta);   const Obj& X = mX;
        		SimpleBlobBufferFactory& fa = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE); 
                bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp42(&fa, &ta);
        		Obj& mX = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp43(&mX, &ta); const Obj& X = mX;
        ...
        if (verbose) cout << "\nTesting blob with different buffer sizes.\n";
        {
            ...
            {
                typedef bdlbb::Blob Obj;
                // Changed: TestBlobBufferFactory fa(&ta, 1);
                // Changed: Obj mX(&fa, &ta);   const Obj& X = mX;
        		TestBlobBufferFactory& fa = *new(ta) TestBlobBufferFactory(1); 
                bslma::RawDeleterProctor<TestBlobBufferFactory, bslma::Allocator> dp44(&fa, &ta);
        		Obj& mX = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp45(&mX, &ta); const Obj& X = mX;
      ...
      case 2: {
        ...
        if (verbose) cout << "\nTesting creating blob without factory.\n";
        {
        	...
        	Obj& mX = *new(ta) Obj(); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp46(&mX, &ta); const Obj& X = mX;
            ...
            // Changed: Obj mY(mX, &ta);  // const Obj& Y = mY;
        	Obj& mY = *new(ta) Obj(mX); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp47(&mY, &ta); const Obj& Y = mY;
        ...
        BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
        {
            ...
            typedef bdlbb::Blob Obj;
            // Changed: SimpleBlobBufferFactory fa(BUFFER_SIZE, &ta);
            // Changed: // Create X, with a factory.
            // Changed: Obj mX(&fa, &ta);   const Obj& X = mX;
        	SimpleBlobBufferFactory& fa = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE); 
            bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp48(&fa, &ta);
        	Obj& mX = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp49(&mX, &ta); const Obj& X = mX;
            ...
            // Changed: Obj mY(mX, &ta);  const Obj& Y = mY;
        	Obj& mY = *new(ta) Obj(mX); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp50(&mY, &ta); const Obj& Y = mY;
            ...
            // Changed: Obj mZ1(mX, &fa, &ta);  const Obj& Z1 = mZ1;
        	Obj& mZ1 = *new(ta) Obj(mX, &fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp51(&mZ1, &ta); const Obj& Z1 = mZ1;
            ...
            // Changed: Obj mZ2(mY, &fa, &ta);  const Obj& Z2 = mZ2;
        	Obj& mZ2 = *new(ta) Obj(mY, &fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp52(&mZ2, &ta); const Obj& Z2 = mZ2;
        ...
        BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
        {
            ...
            // Changed: SimpleBlobBufferFactory fa(BUFFER_SIZE, &ta);
        	SimpleBlobBufferFactory& fa = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE); 
            bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp53(&fa, &ta);
            ...
            // Changed: SimpleBlobBufferFactory fa3(BUFFER_SIZE - 2, &ta);
        	SimpleBlobBufferFactory& fa3 = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE - 2); 
            bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp54(&fa3, &ta);


            bdlbb::BlobBuffer b3; fa3.allocate(&b3);

            // Changed: SimpleBlobBufferFactory fa4(BUFFER_SIZE + 2, &ta);
        	SimpleBlobBufferFactory& fa4 = *new(ta) SimpleBlobBufferFactory(BUFFER_SIZE + 2); 
            bslma::RawDeleterProctor<SimpleBlobBufferFactory, bslma::Allocator> dp55(&fa4, &ta);
            ...
            // Changed: Obj mX(BUFFERS, NUM_BUFFERS, &fa, &ta);  const Obj& X = mX;
        	Obj& mX = *new(ta) Obj(BUFFERS, NUM_BUFFERS, &fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp56(&mX, &ta); const Obj& X = mX;
      ...
      case 1: {
        ...
        {
            typedef bdlbb::Blob Obj;
            // Changed: TestBlobBufferFactory fa(&ta);
            // Changed: Obj mX(&fa, &ta);  const Obj& X = mX;
        	TestBlobBufferFactory& fa = *new(ta) TestBlobBufferFactory(); 
            bslma::RawDeleterProctor<TestBlobBufferFactory, bslma::Allocator> dp57(&fa, &ta);
        	Obj& mX = *new(ta) Obj(&fa); bslma::RawDeleterProctor<Obj, bslma::Allocator> dp58(&mX, &ta); const Obj& X = mX;
