...
// I added these include files - Lester
...
// Lester : Here is where the hard work begins...
// Changed : template <class VALUE_TYPE, class ALLOCATOR = bsl::allocator<VALUE_TYPE> >
template <class VALUE_TYPE>
class Vector_Imp : public Vector_ImpBase<VALUE_TYPE> {
                 // Changed : , private BloombergLP::bslalg::ContainerBase<ALLOCATOR>
    ...
    // Changed : typedef bsl::allocator_traits<ALLOCATOR>                   AllocatorTraits;
    typedef bsl::allocator_traits<bsl::allocator<VALUE_TYPE>>                   AllocatorTraits;
  ...
  public:
    // PUBLIC TYPES
    typedef VALUE_TYPE                                value_type;
    // Changed : typedef ALLOCATOR                                 allocator_type;
    // Changed : typedef typename ALLOCATOR::reference             reference;
    // Changed : typedef typename ALLOCATOR::const_reference       const_reference;
  ...
  private:
    ...
    // Changed : typedef BloombergLP::bslalg::ContainerBase<ALLOCATOR> ContainerBase;
    ...
    class Proctor {
        ...
        // Changed : ContainerBase       *d_container_p;  // container base pointer
      ...
      public:
        ...
        Proctor(VALUE_TYPE    *data,
                std::size_t    capacity);
                // Changed : ContainerBase *container);
  ...
  public:
    ...
    // Changed : explicit Vector_Imp(const ALLOCATOR& basicAllocator = ALLOCATOR());
    explicit
    Vector_Imp();
    ...
    Vector_Imp(size_type         initialSize);
               // Changed : const ALLOCATOR&  basicAllocator = ALLOCATOR());
    ...
    // Changed : Vector_Imp(const Vector_Imp& original, const ALLOCATOR& basicAllocator);
    // Changed : Vector_Imp(const Vector_Imp& original);
        // Create a vector having the same value as the specified 'original'
    ...
    // Changed : Vector_Imp(BloombergLP::bslmf::MovableRef<Vector_Imp> original,
               // Changed : const ALLOCATOR&                           basicAllocator);
        // Create a vector having the same value as the specified 'original'
        // object that uses the specified 'basicAllocator' to supply memory.
...
// }}} END GENERATED CODE
        ...
        // the Sun compiler.
    {
        ...
        if (newSize > this->d_capacity) {
            ...
            // Changed : Vector_Imp temp(this->get_allocator());

            BloombergLP::bslma::Allocator* myAllocator = this->get_allocator();
            Vector_Imp& temp = *new(*myAllocator) Vector_Imp();
            BloombergLP::bslma::RawDeleterProctor<Vector_Imp, BloombergLP::bslma::Allocator> dp1476(&temp, myAllocator);

            temp.privateReserveEmpty(newCapacity);

            using namespace BloombergLP;
            
            bsl::allocator<VALUE_TYPE> myBslAllocator = *new bsl::allocator<VALUE_TYPE>(this->get_allocator());
            ArrayPrimitives::destructiveMoveAndEmplace(
                ...
                pos,
                this->d_dataEnd_p,
                // Changed : ContainerBase::allocator(),
                myBslAllocator,
                BSLS_COMPILERFEATURES_FORWARD(Args, arguments)...);
            delete &myBslAllocator;

            temp.d_dataEnd_p += newSize;
            Vector_Util::swap(&this->d_dataBegin_p, &temp.d_dataBegin_p);

            // Unchanged : this->get_allocator()->deleteObject(&temp);
        }
        else {
...
// }}} END GENERATED CODE
    ...
    // ACCESSORS
    // Changed : allocator_type get_allocator() const BSLS_CPP11_NOEXCEPT;
    BloombergLP::bslma::Allocator* get_allocator() const BSLS_CPP11_NOEXCEPT;
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : bool operator==(const Vector_Imp<VALUE_TYPE, ALLOCATOR>& lhs,
                // Changed : const Vector_Imp<VALUE_TYPE, ALLOCATOR>& rhs);
template <class VALUE_TYPE>
bool operator==(const Vector_Imp<VALUE_TYPE>& lhs,
                const Vector_Imp<VALUE_TYPE>& rhs);
    // Return 'true' if the specified 'lhs' and 'rhs' objects have the same
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR = bsl::allocator<VALUE_TYPE> >
// Changed : class vector : public Vector_Imp<VALUE_TYPE, ALLOCATOR>

template <class VALUE_TYPE>
class vector : public Vector_Imp<VALUE_TYPE>
...
{
    // PRIVATE TYPES
    typedef Vector_Imp<VALUE_TYPE>                  Base;
  ...
  public:
    ...
    // Changed : typedef typename Base::allocator_type         allocator_type;
  ...
  public:
    ...
    // Changed : vector() BSLS_CPP11_NOEXCEPT;
    // Changed : explicit vector(const ALLOCATOR& basicAllocator) BSLS_CPP11_NOEXCEPT;
    vector() BSLS_CPP11_NOEXCEPT;
    // Changed : explicit vector() BSLS_CPP11_NOEXCEPT;
        // Create an empty vector.  Optionally specify a 'basicAllocator' used
        // to supply memory.  If 'basicAllocator' is not specified, a
        // default-constructed object of the (template parameter) type
        // 'ALLOCATOR' is used.  If the type 'ALLOCATOR' is 'bsl::allocator'
    ...
    // Changed : explicit vector(size_type        initialSize,
                    // Changed : const ALLOCATOR& basicAllocator = ALLOCATOR());
    ...
    // Changed : vector(size_type         initialSize,
           // Changed : const VALUE_TYPE& value,
           // Changed : const ALLOCATOR&  basicAllocator = ALLOCATOR());
    ...
    // Changed : template <class INPUT_ITER>
    // Changed : vector(INPUT_ITER       first,
           // Changed : INPUT_ITER       last,
           // Changed : const ALLOCATOR& basicAllocator = ALLOCATOR());
    template <class INPUT_ITER>
    ...
    // Changed : vector(const vector& original, const ALLOCATOR& basicAllocator);
    // Changed : vector(const vector& original);
        // Create a vector having the same value as the specified 'original'
        // object that uses the specified 'basicAllocator' to supply memory.
        // This method requires that the (template parameter) type 'VALUE_TYPE'
        // be 'copy-insertable' into this vector (see {Requirements on
    ...
    // Changed : vector(BloombergLP::bslmf::MovableRef<vector> original,
           // Changed : const ALLOCATOR&                       basicAllocator);
    // Changed : vector(BloombergLP::bslmf::MovableRef<vector> original);
        // Create a vector having the same value as the specified 'original'
        // object that uses the specified 'basicAllocator' to supply memory.
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : bool operator==(const vector<VALUE_TYPE, ALLOCATOR>& lhs,
                // Changed : const vector<VALUE_TYPE, ALLOCATOR>& rhs);
template <class VALUE_TYPE>
bool operator==(const vector<VALUE_TYPE>& lhs,
...
// FREE FUNCTIONS
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : void swap(vector<VALUE_TYPE, ALLOCATOR>& a,
          // Changed : vector<VALUE_TYPE, ALLOCATOR>& b)
             // Changed : BSLS_CPP11_NOEXCEPT_SPECIFICATION(BSLS_CPP11_PROVISIONALLY_FALSE);
template <class VALUE_TYPE>
void swap(vector<VALUE_TYPE>& a,
          vector<VALUE_TYPE>& b)
             BSLS_CPP11_NOEXCEPT_SPECIFICATION(BSLS_CPP11_PROVISIONALLY_FALSE);
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : class vector< VALUE_TYPE *, ALLOCATOR >
// Changed : : public Vector_Imp<void *,
                    // Changed : typename ALLOCATOR::template rebind<void *>::other> 
template <class VALUE_TYPE>
...
: public Vector_Imp<void *>
{
    ...
    // PRIVATE TYPES
    // Changed : typedef typename ALLOCATOR::template rebind<void *>::other BaseAlloc;
    // Changed : typedef Vector_Imp<void *, BaseAlloc>                      Base;
    typedef Vector_Imp<void *>                      Base;
    typedef BloombergLP::bslmf::MovableRefUtil                 MoveUtil;
  ...
  public:
    ...
    typedef std::ptrdiff_t                        difference_type;
    // Changed : typedef ALLOCATOR                             allocator_type;
    // Changed : typedef typename ALLOCATOR::pointer           pointer;
    // Changed : typedef typename ALLOCATOR::const_pointer     const_pointer;
    typedef bsl::reverse_iterator<iterator>       reverse_iterator;
    typedef bsl::reverse_iterator<const_iterator> const_reverse_iterator;
    ...
    // Changed : explicit vector(const ALLOCATOR& basicAllocator) BSLS_CPP11_NOEXCEPT
    // Changed : : Base(BaseAlloc(basicAllocator))

    // Changed : explicit vector(size_type initialSize,
                    // Changed : const ALLOCATOR& basicAllocator = ALLOCATOR())
    // Changed : : Base(initialSize, BaseAlloc(basicAllocator))
    explicit vector(size_type initialSize)
    : Base(initialSize)
    {
    }
    ...
    // Changed : vector(size_type         initialSize,
           // Changed : VALUE_TYPE       *value,
           // Changed : const ALLOCATOR&  basicAllocator = ALLOCATOR())
    // Changed : : Base(initialSize, (void *) value, BaseAlloc(basicAllocator))
    vector(size_type         initialSize,
           VALUE_TYPE       *value)
    : Base(initialSize, (void *) value)
    {
    }
    ...
    // Changed : vector(INPUT_ITER       first,
           // Changed : INPUT_ITER       last,
           // Changed : const ALLOCATOR& basicAllocator = ALLOCATOR())
    // Changed : : Base(first, last, BaseAlloc(basicAllocator))
    template <class INPUT_ITER>
    vector(INPUT_ITER       first,
           INPUT_ITER       last)
    : Base(first, last)
    ...
    // Changed : vector(const vector& original, const ALLOCATOR& basicAllocator)
    // Changed : : Base(original, BaseAlloc(basicAllocator))

    // Changed : vector(BloombergLP::bslmf::MovableRef<vector> original,
           // Changed : const ALLOCATOR& basicAllocator)
    // Changed : : Base(MoveUtil::move(static_cast<Base&>(original)),
           // Changed : BaseAlloc(basicAllocator))

#if defined(BSLS_COMPILERFEATURES_SUPPORT_GENERALIZED_INITIALIZERS)
    // Changed : vector(std::initializer_list<VALUE_TYPE *> values,
           // Changed : const ALLOCATOR&           basicAllocator = ALLOCATOR())
    // Changed : : Base(values.begin(), values.end(), basicAllocator)
    vector(std::initializer_list<VALUE_TYPE *> values)
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : class vector< const VALUE_TYPE *, ALLOCATOR >
// Changed : : public Vector_Imp<const void *,
                    // Changed : typename ALLOCATOR::template rebind<const void *>::other> 

template <class VALUE_TYPE>
class vector< const VALUE_TYPE *>
: public Vector_Imp<const void *>
{
    ...
    // PRIVATE TYPES
    // Changed : typedef typename ALLOCATOR::template rebind<const void *>::other BaseAlloc;
    // Changed : typedef Vector_Imp<const void *, BaseAlloc>                      Base;
    typedef Vector_Imp<const void *>                      Base;
    typedef BloombergLP::bslmf::MovableRefUtil                       MoveUtil;
  ...
  public:
    ...
    typedef const VALUE_TYPE* const & const_reference;
    ...
    // Changed : vector(size_type         initialSize,
           // Changed : const VALUE_TYPE *value,
           // Changed : const ALLOCATOR&  basicAllocator = ALLOCATOR())
    // Changed : : Base(initialSize, (const void *)value, BaseAlloc(basicAllocator))
    vector(size_type         initialSize,
           const VALUE_TYPE *value)
    : Base(initialSize, (const void *)value)
    {
    }
...
#if defined(BSLS_COMPILERFEATURES_SUPPORT_GENERALIZED_INITIALIZERS)
    // Changed : vector(
        // Changed : std::initializer_list<const VALUE_TYPE *> values,
        // Changed : const ALLOCATOR&                          basicAllocator = ALLOCATOR())
    // Changed : : Base(values.begin(), values.end(), basicAllocator)
    vector(
...
# endif
    ...
    BloombergLP::bslma::Allocator* get_allocator() const BSLS_CPP11_NOEXCEPT
    {
        // Changed : return ALLOCATOR(Base::get_allocator());
        return static_cast<BloombergLP::bslma::Allocator*>(BloombergLP::bslma::AllocatorDatabase::lookup(this));
    }
...
// FREE OPERATORS
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : bool operator==(const vector<const VALUE_TYPE *,ALLOCATOR>& lhs,
                // Changed : const vector<const VALUE_TYPE *,ALLOCATOR>& rhs);
template <class VALUE_TYPE>
bool operator==(const vector<const VALUE_TYPE *>& lhs,
                const vector<const VALUE_TYPE *>& rhs);
...
// FREE FUNCTIONS
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : void swap(vector<const VALUE_TYPE *, ALLOCATOR>& a,
          // Changed : vector<const VALUE_TYPE *, ALLOCATOR>& b)
             // Changed : BSLS_CPP11_NOEXCEPT_SPECIFICATION(BSLS_CPP11_PROVISIONALLY_FALSE);
template <class VALUE_TYPE>
void swap(vector<const VALUE_TYPE *>& a,
          vector<const VALUE_TYPE *>& b)
             BSLS_CPP11_NOEXCEPT_SPECIFICATION(BSLS_CPP11_PROVISIONALLY_FALSE);
...
// CREATORS
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : BSLS_PLATFORM_AGGRESSIVE_INLINE
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>::Proctor::Proctor(VALUE_TYPE    *data,
                                                    // Changed : std::size_t    capacity,
                                                    // Changed : ContainerBase *container)
template <class VALUE_TYPE>
BSLS_PLATFORM_AGGRESSIVE_INLINE
Vector_Imp<VALUE_TYPE>::Proctor::Proctor(VALUE_TYPE    *data,
                                                    std::size_t    capacity)
                                                    // Changed : ContainerBase *container)
: d_data_p(data)
, d_capacity(capacity)
// Changed : , d_container_p(container)
{
}

// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : BSLS_PLATFORM_AGGRESSIVE_INLINE
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>::Proctor::~Proctor()
template <class VALUE_TYPE>
BSLS_PLATFORM_AGGRESSIVE_INLINE
Vector_Imp<VALUE_TYPE>::Proctor::~Proctor()
{
    if (d_data_p) {
        // Changed : d_container_p->deallocateN(d_data_p, d_capacity);
        using namespace BloombergLP;
        bslma::Allocator* myAllocator = static_cast<bslma::Allocator*>(bslma::AllocatorDatabase::lookup(this));
        myAllocator->deallocate(d_data_p);
    }
}
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : BSLS_PLATFORM_AGGRESSIVE_INLINE
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::Proctor::release()
template <class VALUE_TYPE>
BSLS_PLATFORM_AGGRESSIVE_INLINE
void Vector_Imp<VALUE_TYPE>::Proctor::release()
{
    d_data_p = 0;
}
...
template <class VALUE_TYPE>
template <class FWD_ITER>
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::constructFromRange(
void Vector_Imp<VALUE_TYPE>::constructFromRange(
                                               FWD_ITER                  first,
                                               FWD_ITER                  last,
                                               std::forward_iterator_tag)
{
    ...
    Proctor proctor(this->d_dataBegin_p,
                    this->d_capacity);
                    // Changed : static_cast<ContainerBase *>(this));
...
template <class INPUT_ITER>
inline
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::privateInsertDispatch(
void Vector_Imp<VALUE_TYPE>::privateInsertDispatch(
                              const_iterator                          position,
                              INPUT_ITER                              count,
                              INPUT_ITER                              value,
                              BloombergLP::bslmf::MatchArithmeticType ,
...
template <class VALUE_TYPE>
template <class INPUT_ITER>
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::privateInsert(
void Vector_Imp<VALUE_TYPE>::privateInsert(
                                      const_iterator                  position,
                                      INPUT_ITER                      first,
                                      INPUT_ITER                      last,
                                      const std::input_iterator_tag&)
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
template <class VALUE_TYPE>
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::privateMoveInsert(
void Vector_Imp<VALUE_TYPE>::privateMoveInsert(
...
template <class VALUE_TYPE>
inline
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::privateReserveEmpty(
void Vector_Imp<VALUE_TYPE>::privateReserveEmpty(
                                                         size_type numElements)
{
    BSLS_ASSERT_SAFE(this->empty());
    BSLS_ASSERT_SAFE(0 == this->capacity());

    // Changed : this->d_dataBegin_p = this->d_dataEnd_p = this->allocateN(
                                                // Changed : (VALUE_TYPE *) 0, numElements);
    using namespace BloombergLP;
    bslma::Allocator* myAllocator = this->get_allocator();
    this->d_dataBegin_p = this->d_dataEnd_p = static_cast<VALUE_TYPE*>(myAllocator->allocate( numElements * sizeof(VALUE_TYPE) ));
...
template <class VALUE_TYPE>
inline
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>::Vector_Imp(const ALLOCATOR& basicAllocator)
Vector_Imp<VALUE_TYPE>::Vector_Imp()
: Vector_ImpBase<VALUE_TYPE>()
// Changed : , ContainerBase(basicAllocator)
{
}

// Changed : template <class VALUE_TYPE, class ALLOCATOR>
template <class VALUE_TYPE>
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>::Vector_Imp(size_type        initialSize)
                                              // Changed : const ALLOCATOR& basicAllocator)
Vector_Imp<VALUE_TYPE>::Vector_Imp(size_type        initialSize)
: Vector_ImpBase<VALUE_TYPE>()
// Changed : , ContainerBase(basicAllocator)
{
...
template <class INPUT_ITER>
BSLS_PLATFORM_AGGRESSIVE_INLINE
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>::Vector_Imp(INPUT_ITER       first,
Vector_Imp<VALUE_TYPE>::Vector_Imp(INPUT_ITER       first,
                                              INPUT_ITER       last)
                                              // Changed : const ALLOCATOR& basicAllocator)
: Vector_ImpBase<VALUE_TYPE>()
// Changed : , ContainerBase(basicAllocator)
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
template <class VALUE_TYPE>
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>::Vector_Imp(const Vector_Imp& original)
Vector_Imp<VALUE_TYPE>::Vector_Imp(const Vector_Imp& original)
: Vector_ImpBase<VALUE_TYPE>()
// Changed : , ContainerBase(AllocatorTraits::select_on_container_copy_construction(
                                          // Changed : original.ContainerBase::allocator()))
{
    if (original.size() > 0) {
        ...
        ArrayPrimitives::copyConstruct(this->d_dataBegin_p,
                                       original.begin(),
                                       original.end(),
                                       // Chagned : ContainerBase::allocator());
                                       this->get_allocator());
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
template <class VALUE_TYPE>
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>::Vector_Imp(
Vector_Imp<VALUE_TYPE>::Vector_Imp(
                           BloombergLP::bslmf::MovableRef<Vector_Imp> original)
: Vector_ImpBase<VALUE_TYPE>()
// Changed : , ContainerBase(MoveUtil::access(original).get_allocator())
{
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>::Vector_Imp(
// Changed : Vector_Imp<VALUE_TYPE>::Vector_Imp(
                     // Changed : BloombergLP::bslmf::MovableRef<Vector_Imp> original,
                     // Changed : const ALLOCATOR&                           basicAllocator)
// Changed : : Vector_ImpBase<VALUE_TYPE>()
// Changed : , ContainerBase(basicAllocator)
// Changed : {
    // Changed : Vector_Imp& lvalue = original;

    // Changed : if (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(get_allocator() ==
                                            // Changed : lvalue.get_allocator())) {
        // Changed : ImpBase::adopt(MoveUtil::move(static_cast<ImpBase&>(lvalue)));
    // Changed : }
    // Changed : else {
        // Changed : if (lvalue.size() > 0) {
            // Changed : privateReserveEmpty(lvalue.size());
            // Changed : Proctor proctor(this->d_dataBegin_p,
                            // Changed : this->d_capacity,
                            // Changed : static_cast<ContainerBase *>(this));

            // Changed : ArrayPrimitives::moveConstruct(this->d_dataBegin_p,
                                           // Changed : lvalue.begin(),
                                           // Changed : lvalue.end(),
                                           // Changed : ContainerBase::allocator());

            // Changed : proctor.release();
            // Changed : this->d_dataEnd_p += lvalue.size();
        // Changed : }
    // Changed : }
// Changed : }
...
Vector_Imp<VALUE_TYPE>::~Vector_Imp()
{
    if (this->d_dataBegin_p) {
        ...
        BloombergLP::bslalg::ArrayDestructionPrimitives::destroy(
                                            ...
                                            this->d_dataEnd_p,
                                            // Changed : ContainerBase::allocator());
                                            // Changed : this->get_allocator());
                                            myBslAllocator);
        // Changed : this->deallocateN(this->d_dataBegin_p, this->d_capacity);
        this->get_allocator()->deallocate(this->d_dataBegin_p);
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
template <class VALUE_TYPE>
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>&
Vector_Imp<VALUE_TYPE>&
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>::operator=(const Vector_Imp& rhs)
Vector_Imp<VALUE_TYPE>::operator=(const Vector_Imp& rhs)
{
    if (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(this != &rhs)) {
        // Changed : if (AllocatorTraits::propagate_on_container_copy_assignment::value) {
            // Changed : Vector_Imp other(rhs, rhs.get_allocator());
            // Changed : Vector_Util::swap(&this->d_dataBegin_p, &other.d_dataBegin_p);
            // Changed : using std::swap;
            // Changed : swap(ContainerBase::allocator(), other.ContainerBase::allocator());
            // ^ We can't swap allocators in the new model!
        // Changed : }
        // Changed : else {
...
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>&
Vector_Imp<VALUE_TYPE>&
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>::operator=(
Vector_Imp<VALUE_TYPE>::operator=(
                                BloombergLP::bslmf::MovableRef<Vector_Imp> rhs)
{
    ...
    if (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(this != &lvalue)) {
        ...
        // Changed : else if (
              // Changed : AllocatorTraits::propagate_on_container_move_assignment::value) {
            // Changed : Vector_Imp other(MoveUtil::move(lvalue));
            // Changed : using std::swap;
            // Changed : swap(ContainerBase::allocator(), other.ContainerBase::allocator());
            // Changed : Vector_Util::swap(&this->d_dataBegin_p, &other.d_dataBegin_p);
        // Changed : }
            ...
            BloombergLP::bslma::Allocator* myAllocator = this->get_allocator();
            Vector_Imp& other = *new(*myAllocator) Vector_Imp(MoveUtil::move(lvalue));
            BloombergLP::bslma::RawDeleterProctor<Vector_Imp, BloombergLP::bslma::Allocator> dp1476(&other, myAllocator);
...
template <class INPUT_ITER>
inline
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::assign(INPUT_ITER first,
void Vector_Imp<VALUE_TYPE>::assign(INPUT_ITER first,
                                               INPUT_ITER last)
{
    BSLS_ASSERT_SAFE(!Vector_RangeCheck::isInvalidRange(first, last));
...
template <class VALUE_TYPE>
inline
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::assign(size_type         numElements,
void Vector_Imp<VALUE_TYPE>::assign(size_type         numElements,
                                               const VALUE_TYPE& value)
{
    if (!this->empty()) {
        erase(this->begin(), this->end());
...
template <class VALUE_TYPE>
inline
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::assign(
void Vector_Imp<VALUE_TYPE>::assign(
                                      std::initializer_list<VALUE_TYPE> values)
{
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
template <class VALUE_TYPE>
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::resize(size_type newSize)
void Vector_Imp<VALUE_TYPE>::resize(size_type newSize)
{
    ...
    if (newSize <= this->size()) {
        ...
        // Changed : Vector_Imp temp(newSize, this->get_allocator());

        BloombergLP::bslma::Allocator* myAllocator = this->get_allocator();
        Vector_Imp& temp = *new(*myAllocator) Vector_Imp(newSize);
        BloombergLP::bslma::RawDeleterProctor<Vector_Imp, BloombergLP::bslma::Allocator> dp1476(&temp, myAllocator);
        // Vector_Imp temp(newSize);
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
template <class VALUE_TYPE>
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::reserve(size_type newCapacity)
void Vector_Imp<VALUE_TYPE>::reserve(size_type newCapacity)
{
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
template <class VALUE_TYPE>
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::shrink_to_fit()
void Vector_Imp<VALUE_TYPE>::shrink_to_fit()
{
...
template <class... Args>
inline
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::emplace_back(Args&&...arguments)
void Vector_Imp<VALUE_TYPE>::emplace_back(Args&&...arguments)
{
    if (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(this->d_capacity > this->size())) {

        using namespace BloombergLP;
        bsl::allocator<VALUE_TYPE>& myAllocator = *new bsl::allocator<VALUE_TYPE>(this->get_allocator());
        ...
        // moves.
        Vector_PushProctor<VALUE_TYPE, bsl::allocator<VALUE_TYPE>> guard(
                                                   pos,
                                                   // Changed : ContainerBase::allocator());
                                                   this->get_allocator());
        ArrayPrimitives::destructiveMove(temp.d_dataBegin_p,
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
template <class VALUE_TYPE>
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::push_back(const VALUE_TYPE& value)
void Vector_Imp<VALUE_TYPE>::push_back(const VALUE_TYPE& value)
{
    if (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(this->d_capacity > this->size())) {
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
template <class VALUE_TYPE>
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::push_back(
void Vector_Imp<VALUE_TYPE>::push_back(
                              BloombergLP::bslmf::MovableRef<VALUE_TYPE> value)
{
    ...
    if (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(this->d_capacity > this->size())) {
        ...
        // 'construct' call for types with potentially-throwing destructive
        // moves.
        // Changed : Vector_PushProctor<VALUE_TYPE, ALLOCATOR> guard(
        Vector_PushProctor<VALUE_TYPE, bsl::allocator<VALUE_TYPE>> guard(
                                                   pos,
                                                   // Changed : ContainerBase::allocator());
                                                   this->get_allocator());
...
template <class VALUE_TYPE>
inline
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::pop_back()
void Vector_Imp<VALUE_TYPE>::pop_back()
{
    BSLS_ASSERT_SAFE(!this->empty());

    // Changed : AllocatorTraits::destroy(ContainerBase::allocator(),
                             // Changed : --this->d_dataEnd_p);
    bsl::allocator<VALUE_TYPE>& myBslAllocator = *new bsl::allocator<VALUE_TYPE>(this->get_allocator());
    AllocatorTraits::destroy(myBslAllocator,
                             --this->d_dataEnd_p);
    delete &myBslAllocator;
}
...
template <class VALUE_TYPE>
inline
// Changed : typename Vector_Imp<VALUE_TYPE, ALLOCATOR>::iterator
typename Vector_Imp<VALUE_TYPE>::iterator
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>::insert(const_iterator    position,
Vector_Imp<VALUE_TYPE>::insert(const_iterator    position,
                                          const VALUE_TYPE& value)
{
    BSLS_ASSERT_SAFE(this->begin() <= position);
    BSLS_ASSERT_SAFE(position      <= this->end());
...
// Changed : typename Vector_Imp<VALUE_TYPE, ALLOCATOR>::iterator
typename Vector_Imp<VALUE_TYPE>::iterator
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>::insert(
Vector_Imp<VALUE_TYPE>::insert(
                           const_iterator                             position,
                           BloombergLP::bslmf::MovableRef<VALUE_TYPE> value)
{
    BSLS_ASSERT_SAFE(this->begin() <= position);
...
// Changed : typename Vector_Imp<VALUE_TYPE, ALLOCATOR>::iterator
typename Vector_Imp<VALUE_TYPE>::iterator
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>::erase(const_iterator position)
Vector_Imp<VALUE_TYPE>::erase(const_iterator position)
{
    BSLS_ASSERT_SAFE(this->begin() <= position);
    BSLS_ASSERT_SAFE(position      <  this->end());
...
// Changed : typename Vector_Imp<VALUE_TYPE, ALLOCATOR>::iterator
typename Vector_Imp<VALUE_TYPE>::iterator
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>::erase(const_iterator first,
Vector_Imp<VALUE_TYPE>::erase(const_iterator first,
                                         const_iterator last)
{
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
template <class VALUE_TYPE>
// Changed : void Vector_Imp<VALUE_TYPE, ALLOCATOR>::swap(
void Vector_Imp<VALUE_TYPE>::swap(
                                      // Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>& other)
                                    Vector_Imp<VALUE_TYPE>& other)
              BSLS_CPP11_NOEXCEPT_SPECIFICATION(BSLS_CPP11_PROVISIONALLY_FALSE)
{
    // Lester : LOOK AT THIS
    // Changed : if (AllocatorTraits::propagate_on_container_swap::value) {
        // Changed : Vector_Util::swap(&this->d_dataBegin_p, &other.d_dataBegin_p);
        // Changed : using std::swap;
        // Changed : swap(ContainerBase::allocator(), other.ContainerBase::allocator());
    // Changed : }
    // Changed : else {
        if (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(
            ...
            // Lester : I added this (we can only move data that are allocated by the same allocator)
            if (other.get_allocator() != this->get_allocator()) return;

            using namespace BloombergLP;

            // Changed : Vector_Imp toOtherCopy(MoveUtil::move(*this),
                                   // Changed : other.get_allocator());

            bslma::Allocator* thisAllocator = this->get_allocator();

            Vector_Imp& toOtherCopy = *new(*thisAllocator) Vector_Imp(MoveUtil::move(*this)); 
            bslma::RawDeleterProctor<Vector_Imp, bslma::Allocator> dp4636(&toOtherCopy, thisAllocator);

            // Changed : Vector_Imp toThisCopy( MoveUtil::move(other),
                                   // Changed : this->get_allocator());

            Vector_Imp& toThisCopy = *new(*thisAllocator) Vector_Imp(MoveUtil::move(*this)); 
            bslma::RawDeleterProctor<Vector_Imp, bslma::Allocator> dp4642(&toThisCopy, thisAllocator);
...
template <class VALUE_TYPE>
inline
// Changed: void Vector_Imp<VALUE_TYPE, ALLOCATOR>::clear() BSLS_CPP11_NOEXCEPT
void Vector_Imp<VALUE_TYPE>::clear() BSLS_CPP11_NOEXCEPT
{
...
template <class VALUE_TYPE>
inline
// Changed : typename Vector_Imp<VALUE_TYPE, ALLOCATOR>::allocator_type
BloombergLP::bslma::Allocator*
// Changed : Vector_Imp<VALUE_TYPE, ALLOCATOR>::get_allocator() const BSLS_CPP11_NOEXCEPT
Vector_Imp<VALUE_TYPE>::get_allocator() const BSLS_CPP11_NOEXCEPT
{
    // Changed : return ContainerBase::allocator();
    return static_cast<BloombergLP::bslma::Allocator*>(BloombergLP::bslma::AllocatorDatabase::lookup(this));
}
...
template <class VALUE_TYPE>
inline
// Changed : typename Vector_Imp<VALUE_TYPE, ALLOCATOR>::size_type
typename Vector_Imp<VALUE_TYPE>::size_type
Vector_Imp<VALUE_TYPE>::max_size() const BSLS_CPP11_NOEXCEPT
{
    // Changed : return ContainerBase::allocator().max_size();
    return INT_MAX;
}
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : inline
// Changed : vector<VALUE_TYPE, ALLOCATOR>::vector() BSLS_CPP11_NOEXCEPT
// Changed : : Base(ALLOCATOR())
template <class VALUE_TYPE>
inline
vector<VALUE_TYPE>::vector() BSLS_CPP11_NOEXCEPT
: Base()
{
}
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : inline
// Changed : vector<VALUE_TYPE, ALLOCATOR>::vector(const ALLOCATOR& basicAllocator)
                                                            // Changed : BSLS_CPP11_NOEXCEPT
// Changed : Base(basicAllocator)

// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : inline
// Changed : vector<VALUE_TYPE, ALLOCATOR>::vector(size_type        initialSize,
                                      // Changed : const ALLOCATOR& basicAllocator)
// Changed : : Base(initialSize, basicAllocator)
template <class VALUE_TYPE>
inline
vector<VALUE_TYPE>::vector(size_type        initialSize)
: Base(initialSize)
{
}
...
// Changed : vector<VALUE_TYPE, ALLOCATOR>::vector(size_type         initialSize,
                                      // Changed : const VALUE_TYPE& value,
                                      // Changed : const ALLOCATOR&  basicAllocator)
// Changed : : Base(initialSize, value, basicAllocator)
template <class VALUE_TYPE>
inline
vector<VALUE_TYPE>::vector(size_type         initialSize,
                                      const VALUE_TYPE& value)
: Base(initialSize, value)
{
}
...
// Changed : template <class INPUT_ITER>
// Changed : inline
// Changed : vector<VALUE_TYPE, ALLOCATOR>::vector(INPUT_ITER       first,
                                      // Changed : INPUT_ITER       last,
                                      // Changed : const ALLOCATOR& basicAllocator)
// Changed : : Base(first, last, basicAllocator)
template <class VALUE_TYPE>
template <class INPUT_ITER>
inline
vector<VALUE_TYPE>::vector(INPUT_ITER       first,
                                      INPUT_ITER       last)
: Base(first, last)
{
}
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : inline
// Changed : vector<VALUE_TYPE, ALLOCATOR>::vector(
                              // Changed : std::initializer_list<VALUE_TYPE> values,
                              // Changed : const ALLOCATOR&                  basicAllocator)
// Changed : : Base(values.begin(), values.end(), basicAllocator)
template <class VALUE_TYPE>
inline
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : inline
// Changed : vector<VALUE_TYPE, ALLOCATOR>::vector(const vector& original)
// Changed : : Base(original)
template <class VALUE_TYPE>
inline
vector<VALUE_TYPE>::vector(const vector& original)
: Base(original)
{
}
...
// Changed : inline
// Changed : vector<VALUE_TYPE, ALLOCATOR>::vector(
                               // Changed : BloombergLP::bslmf::MovableRef<vector> original)
                                                            // Changed : BSLS_CPP11_NOEXCEPT
// Changed : : Base(MoveUtil::move(static_cast<Base&>(original)))
template <class VALUE_TYPE>
inline
...
// Changed : vector<VALUE_TYPE, ALLOCATOR>::vector(const vector&    original,
                                      // Changed : const ALLOCATOR& basicAllocator)
// Changed : : Base(original, basicAllocator)
...
// Changed : vector<VALUE_TYPE, ALLOCATOR>::vector(
                         // Changed : BloombergLP::bslmf::MovableRef<vector> original,
                         // Changed : const ALLOCATOR&                       basicAllocator)
// Changed : : Base(MoveUtil::move(static_cast<Base&>(original)), basicAllocator)
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : inline
// Changed : vector<VALUE_TYPE, ALLOCATOR>&
// Changed : vector<VALUE_TYPE, ALLOCATOR>::operator=(const vector& rhs)
template <class VALUE_TYPE>
inline
vector<VALUE_TYPE>&
vector<VALUE_TYPE>::operator=(const vector& rhs)
{
    Base::operator=(rhs);
    return *this;
}
...
// Changed : inline
// Changed : vector<VALUE_TYPE, ALLOCATOR>&
// Changed : vector<VALUE_TYPE, ALLOCATOR>::operator=(
                                    // Changed : BloombergLP::bslmf::MovableRef<vector> rhs)
              // Changed : BSLS_CPP11_NOEXCEPT_SPECIFICATION(BSLS_CPP11_PROVISIONALLY_FALSE)
template <class VALUE_TYPE>
inline
vector<VALUE_TYPE>&
...
// Changed : template <class HASHALG, class VALUE_TYPE, class ALLOCATOR>
// Changed : inline
// Changed : void hashAppend(HASHALG& hashAlg, const vector<VALUE_TYPE, ALLOCATOR>& input)
template <class HASHALG, class VALUE_TYPE>
inline
void hashAppend(HASHALG& hashAlg, const vector<VALUE_TYPE>& input)
{
    using ::BloombergLP::bslh::hashAppend;
    // Changed : typedef typename vector<VALUE_TYPE, ALLOCATOR>::const_iterator ci_t;
    typedef typename vector<VALUE_TYPE>::const_iterator ci_t;
                ...
                const vector<const VALUE_TYPE *>& rhs)
{
    ...
    // Changed : typedef typename ALLOCATOR::template rebind<VoidType *>::other BaseAlloc;
    // Changed : typedef Vector_Imp<VoidType *, BaseAlloc>                      Base;
    typedef Vector_Imp<VoidType *>                      Base;
#else
    // Changed : typedef typename ALLOCATOR::template rebind<const void *>::other BaseAlloc;
    // Changed : typedef Vector_Imp<const void *, BaseAlloc>                      Base;
    typedef Vector_Imp<const void *>                      Base;
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : struct HasStlIterators<bsl::vector<VALUE_TYPE, ALLOCATOR> > : bsl::true_type
template <class VALUE_TYPE>
struct HasStlIterators<bsl::vector<VALUE_TYPE> > : bsl::true_type
{};
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : struct IsBitwiseMoveable<bsl::vector<VALUE_TYPE, ALLOCATOR> >
    // Changed : IsBitwiseMoveable<ALLOCATOR>


template <class VALUE_TYPE>
struct IsBitwiseMoveable<bsl::vector<VALUE_TYPE> >
        : IsBitwiseMoveable<typename bsl::allocator<VALUE_TYPE>>
{
...
// Changed : template <class VALUE_TYPE, class ALLOCATOR>
// Changed : struct UsesBslmaAllocator<bsl::vector<VALUE_TYPE, ALLOCATOR> >
    // Changed : : bsl::is_convertible<Allocator *, ALLOCATOR>::type
template <class VALUE_TYPE>
struct UsesBslmaAllocator<bsl::vector<VALUE_TYPE> >
: bsl::is_convertible<bslma::Allocator *, bsl::allocator<VALUE_TYPE>>::type
{};
