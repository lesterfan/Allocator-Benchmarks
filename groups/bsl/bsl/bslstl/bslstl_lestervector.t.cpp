...
//..
  ...
  public:
    ...
    MyMatrix(int               numRows,
             int               numColumns);
             // Changed : bslma::Allocator *basicAllocator = 0);
...
MyMatrix<TYPE>::MyMatrix(int numRows,
                         int numColumns)
: d_matrix(numRows), d_numColumns(numColumns)
                         // Changed : bslma::Allocator *basicAllocator)
// Changed : : d_matrix(numRows, basicAllocator)
// Changed : , d_numColumns(numColumns)
{
...
: d_matrix(original.d_matrix), d_numColumns(original.d_numColumns)
                         // Changed : bslma::Allocator *basicAllocator)
// Changed : : d_matrix(original.d_matrix, basicAllocator)
// Changed : , d_numColumns(original.d_numColumns)
{
}
//..
...
// Changed : template <class TYPE, class ALLOC>
// Changed : void debugprint(const bsl::Vector_Imp<TYPE, ALLOC>& v)
template <class TYPE>
void debugprint(const bsl::Vector_Imp<TYPE>& v)
{
    if (v.empty()) {
        printf("<empty>");
    }
...
// Changed : template <class TYPE, class ALLOC>
// Changed : inline
// Changed : void debugprint(const bsl::vector<TYPE, ALLOC>& v)
template <class TYPE>
inline
void debugprint(const bsl::vector<TYPE>& v)
{
    if (v.empty()) {
        printf("<empty>");
    }
...
// Changed : template <class TYPE, class ALLOC>
// Changed : void primaryManipulator(bsl::vector<TYPE, ALLOC> *container,
                        // Changed : int                       identifier)
template <class TYPE>
void primaryManipulator(bsl::vector<TYPE> *container,
                        int                       identifier)
...
// Changed : template <class TYPE, class ALLOC>
// Changed : void resetMovedInto(bsl::vector<TYPE, ALLOC> *object)
template <class TYPE>
void resetMovedInto(bsl::vector<TYPE> *object)
{
    typedef bsltf::TemplateTestFacility TstFacility;
...
// Changed : template <class TYPE, class ALLOC>
// Changed : int ggg(bsl::vector<TYPE, ALLOC> *object,
        // Changed : const char               *spec,
        // Changed : bool                      verboseFlag = true)
template <class TYPE>
int ggg(bsl::vector<TYPE> *object,
        const char               *spec,
...
// Changed : template <class TYPE, class ALLOC>
// Changed : bsl::vector<TYPE, ALLOC>& gg(bsl::vector<TYPE, ALLOC> *object,
                             // Changed : const char               *spec)
template <class TYPE>
bsl::vector<TYPE>& gg(bsl::vector<TYPE> *object,
                             const char               *spec)
...
struct ExceptionProctor {
  ...
  public:
    ...
    {
        if (d_object_p) {
            // Changed : new (d_control.buffer()) OBJECT(*object, basicAllocator);
            new (d_control.buffer()) OBJECT(*object);
        }
    }
    ...
    {
        if (d_object_p) {
            // Changed : new (d_control.buffer()) OBJECT(control, basicAllocator);
            new (d_control.buffer()) OBJECT(control);
        }
    }
...
// Unchanged : template <class TYPE>

template <class TYPE, class ALLOC = bsl::allocator<TYPE> >
struct TestDriver {
    // TYPES
    typedef bsl::vector<TYPE>             Obj;
    ...
    typedef bsl::allocator_traits<bsl::allocator<TYPE>>         AllocatorTraits;
...
void TestDriver<TYPE, ALLOC>::stretch(Obj         *object,
                                      ...
                                      int          identifier)
// Unchanged : template <class TYPE>
// Unchanged : void TestDriver<TYPE>::stretch(Obj         *object,
                                      // Unchanged : std::size_t  size,
                                      // Unchanged : int          identifier)
{
    ASSERT(object);
...
void TestDriver<TYPE, ALLOC>::stretchRemoveAll(Obj         *object,
                                               ...
                                               int          identifier)
// Unchanged : template <class TYPE>
// Unchanged : void TestDriver<TYPE>::stretchRemoveAll(Obj         *object,
                                               // Unchanged : std::size_t  size,
                                               // Unchanged : int          identifier)
{
    ASSERT(object);
...
// Unchanged : template <class TYPE>
// Unchanged : void TestDriver<TYPE>::testCase11()
{
    // ------------------------------------------------------------------------
    // TEST ALLOCATOR-RELATED CONCERNS
    //
    ...
    // Changed : (not needed) ASSERT((bsl::is_convertible<bslma::Allocator *,
                                // Changed : (not needed) typename Obj::allocator_type>::value));
    // Changed : (not needed) ASSERT((bslma::UsesBslmaAllocator<Obj>::value));
        ...
        printf("\tTesting that empty vector does not allocate.\n");
    {
        // Changed : Obj mX(&ta);
        Obj& mX = *new(ta) Obj();
        bslma::RawDeleterProctor<Obj, bslma::Allocator> dp1587(&mX, &ta);

        // Changed : (not true anymore) ASSERT(0 == ta.numBytesInUse());
    ...
    // Changed : ASSERT(( bslma::UsesBslmaAllocator<TYPE>::value));
    {
        if (verbose) printf("\tTest A!\n");
        // Note that this constructor has not otherwise been tested yet
        // Changed : Obj mX(1, VALUES[0], &ta);  const Obj& X = mX;
        Obj& mX = *new(ta) Obj(1, VALUES[0]);
        bslma::RawDeleterProctor<Obj, bslma::Allocator> dp1601(&mX, &ta);
        const Obj& X = mX;

        // Changed : ASSERT(&ta == X[0].allocator());
        // Changed : ASSERT(2   == ta.numBlocksInUse());
        ASSERT(&ta == bslma::AllocatorDatabase::lookup(&X[0]));
        // Changed : ASSERT(3   == ta.numBlocksInUse());
    }
    {
        if (verbose) printf("\tTest B!\n");
        // Changed : Obj mX(&ta);  const Obj& X = mX;
        Obj& mX = *new(ta) Obj();
        bslma::RawDeleterProctor<Obj, bslma::Allocator> dp1612(&mX, &ta);
        const Obj& X = mX;
...
// Unchanged : template <class TYPE>
// Unchanged : template <bool PROPAGATE_ON_CONTAINER_COPY_ASSIGNMENT_FLAG,
          // Changed : bool OTHER_FLAGS>
// Unchanged : void TestDriver<TYPE>::
                    // Changed : testCase9_propagate_on_container_copy_assignment_dispatch()
{
    ...
    // Changed : typedef bsl::vector<TYPE, StdAlloc>         Obj;
    typedef bsl::vector<TYPE>         Obj;
    ...
    for (int ti = 0; ti < NUM_SPECS; ++ti) {
        ...
        // Changed : StdAlloc mas(&oas);
        // Changed : StdAlloc mat(&oat);

        // Changed : StdAlloc scratch(&da);

        // Changed : Obj mW(scratch);
        // Changed : const Obj& W = gg(&mW, ISPEC);  // control
        Obj& mW = *new(da) Obj();
        bslma::RawDeleterProctor<Obj, bslma::Allocator> dp1679(&mW, &da);
        ...
        for (int tj = 0; tj < NUM_SPECS; ++tj) {
            ...
            TestValues        JVALUES(JSPEC);
            {
                ...
                // Changed : Obj        mY(mas);
                Obj& mY = *new(oas) Obj();
                bslma::RawDeleterProctor<Obj, bslma::Allocator> dp1694(&mY, &oas);
                ...
                // Changed : Obj        mX(mat);
                Obj& mX = *new(oat) Obj();
                bslma::RawDeleterProctor<Obj, bslma::Allocator> dp1702(&mX, &oat);
                ...
                // Changed : ASSERTV(ISPEC, JSPEC, PROPAGATE,
                       // Changed : !PROPAGATE == (mat == X.get_allocator()));
                // Changed : ASSERTV(ISPEC, JSPEC, PROPAGATE,
                        // Changed : PROPAGATE == (mas == X.get_allocator()));

                // Changed : ASSERTV(ISPEC, JSPEC, mas == Y.get_allocator());

                if (PROPAGATE) {
                    // Changed : ASSERTV(ISPEC, JSPEC, 0 == oat.numBlocksInUse());
                }
                else {
...
// Unchanged : template <class TYPE>
// Unchanged : void TestDriver<TYPE>::
                             // Unchanged : testCase9_propagate_on_container_copy_assignment()
{
    // ------------------------------------------------------------------------
...
// Unchanged : template <class TYPE>
// Unchanged : void TestDriver<TYPE>::testCase9()
{
    // ------------------------------------------------------------------------
                ...
                for (int vi = 0; vi < NUM_SPECS; ++vi) {
                    ...
                    for (int uj = 0; uj < NUM_EXTEND; ++uj) {
                        ...
                        for (int vj = 0; vj < NUM_EXTEND; ++vj) {
                            ...
                            // Changed : Obj mU(xta);
                            Obj& mU = *new(ta) Obj();
                            bslma::RawDeleterProctor<Obj, bslma::Allocator> dp1932(&mU, &ta);
                            ...
                            const Obj& U = gg(&mU, U_SPEC);
                            {
                                // Changed : Obj mV(xta);
                                Obj& mV = *new(ta) Obj();
                                bslma::RawDeleterProctor<Obj, bslma::Allocator> dp1942(&mV, &ta);
        ...
        int iteration = 0;
        {
            ...
            for (int ui = 0; ui < NUM_SPECS; ++ui) {
                ...
                for (int vi = 0; vi < NUM_SPECS; ++vi) {
                    ...
                    // Changed : Obj mU(xta);
                    Obj& mU = *new(ta) Obj();
                    bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2054(&mU, &ta);
                    ...
                    const Obj& U = gg(&mU, U_SPEC);
                    {
                        // Changed : Obj mV(xta);
                        Obj& mV = *new(ta) Obj();
                        bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2063(&mV, &ta);
        ...
        for (int ti = 0;  ti < NUM_SPECS; ++ti) {
            ...
            for (int tj = 0; tj < NUM_EXTEND; ++tj) {
                BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(ta) {
                    ...
                    // Changed : Obj mY(xta);
                    Obj& mY = *new(ta) Obj();
                    bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2054(&mY, &ta);
...
{
    // Changed : I removed this test case because it did not apply to the new allocator model.
}

template <class TYPE, class ALLOC>
void TestDriver<TYPE, ALLOC>::testCase8_propagate_on_container_swap()
// Unchanged : template <class TYPE>
// Unchanged : void TestDriver<TYPE>::testCase8_propagate_on_container_swap()
{
    // ------------------------------------------------------------------------
    ...
    // REMOVED THIS TEST : NOT APPLICABLE IN THE NEW MODEL - LESTER

    if (verbose) printf("\t'propagate_on_container_swap::value == false'\n");

    // Changed : testCase8_propagate_on_container_swap_dispatch<false, false>();
    // Changed : testCase8_propagate_on_container_swap_dispatch<false, true>();

    if (verbose) printf("\t'propagate_on_container_swap::value == true'\n");

    // Changed : testCase8_propagate_on_container_swap_dispatch<true, false>();
    // Changed : testCase8_propagate_on_container_swap_dispatch<true, true>();
}

template <class TYPE, class ALLOC>
void TestDriver<TYPE, ALLOC>::testCase8_dispatch()
// Unchanged : template <class TYPE>
// Unchanged : void TestDriver<TYPE>::testCase8_dispatch()
{
    // ------------------------------------------------------------------------
    ...
    //:   also be swapped.
    //:
    //: 7 Both functions provides the strong exception guarantee w.r.t. to
    //:   memory allocation.
    //:
    ...
    {
        // Changed : typedef bsl::Vector_Imp<TYPE, ALLOC> IObj;
        typedef bsl::Vector_Imp<TYPE> IObj;
    ...
    // Changed : ALLOC xda(&da);
    // Changed : ALLOC xoa(&oa);
    // Changed : ALLOC xza(&za);
    // Changed : ALLOC xsa(&sa);
    ...
    // Changed : ASSERT((otherTraitsSet ? xsa : xda) ==
                  // Changed : AllocatorTraits::select_on_container_copy_construction(xsa));
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        // Changed : Obj mW(xoa);     const Obj& W  = gg(&mW,  SPEC1);
        // Changed : Obj mXX(xsa);    const Obj& XX = gg(&mXX, SPEC1);
        Obj& mW = *new(oa) Obj();
        bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2544(&mW, &oa);
        const Obj& W  = gg(&mW,  SPEC1);
        Obj& mXX = *new(sa) Obj();
        bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2548(&mXX, &sa);
        ...
        if (0 == ti) {
            // Changed : ASSERTV(SPEC1, Obj(xsa), W, Obj(xsa) == W);
        }
        ...
        for (int member = 0; member < 2; ++member) {
            ...
            // Changed : ASSERTV(SPEC1, xoa == W.get_allocator());
            ASSERTV(SPEC1, &oa == W.get_allocator());

            // Changed : ASSERTV(SPEC1, oam.isTotalSame());
            // ^ Is false in the new model
        }

        for (int tj = 0; tj < NUM_DATA; ++tj) {
            const char *const SPEC2   = DATA[tj].d_spec;

            // Changed : Obj mYY(xsa);   const Obj& YY = gg(&mYY, SPEC2);
            Obj& mYY = *new(sa) Obj();
            bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2584(&mYY, &sa);
            const Obj& YY = gg(&mYY, SPEC2);
            ...
            for (int member = 0; member < 2; ++member) {
                // Changed : Obj mX(xoa);    const Obj& X  = gg(&mX,  SPEC1);
                // Changed : Obj mY(xoa);    const Obj& Y  = gg(&mY,  SPEC2);
                Obj& mX = *new(oa) Obj();
                bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2593(&mX, &oa);
                const Obj& X  = gg(&mX,  SPEC1);

                Obj& mY = *new(oa) Obj();
                bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2597(&mY, &oa);
                ...
                // Changed : ASSERTV(SPEC1, SPEC2, xoa == X.get_allocator());
                // Changed : ASSERTV(SPEC1, SPEC2, xoa == Y.get_allocator());
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        // Changed : Obj mXX(xsa);    const Obj& XX = gg(&mXX, SPEC1);
        Obj& mXX = *new(sa) Obj();
        bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2642(&mXX, &sa);
        const Obj& XX = gg(&mXX, SPEC1);
        ...
        for (int tj = 0; tj < NUM_DATA; ++tj) {
            ...
            // Changed : Obj mYY(xsa);   const Obj& YY = gg(&mYY, SPEC2);
            Obj& mYY = *new(sa) Obj();
            bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2642(&mYY, &sa);
            const Obj& YY = gg(&mYY, SPEC2);
            ...
            for (int member = 0; member < 2; ++member) {
                ...
                BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(oa) {
                    ...
                    // Changed : Obj mX(xoa);    const Obj& X = gg(&mX, SPEC1);
                    // Changed : Obj mZ(xza);    const Obj& Z = gg(&mZ, SPEC2);
                    Obj& mX = *new(oa) Obj();
                    bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2642(&mX, &oa);
                    const Obj& X = gg(&mX, SPEC1);

                    // Changed : Obj& mZ = *new(za) Obj();
                    // Changed : bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2674(&mZ, &za);
                    // Changed : const Obj& Z = gg(&mZ, SPEC2);
                    Obj& mZ = *new(oa) Obj();
                    bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2674(&mZ, &oa);
                    ...
                    // Changed : ASSERTV(SPEC1, SPEC2, YY, X, YY == X);
                    ASSERTV(SPEC1, SPEC2, YY, X, mX, mZ, YY == X);

                    ASSERTV(SPEC1, SPEC2, XX, Z, XX == Z);
                    // Changed : ASSERTV(SPEC1, SPEC2, (isPropagate ? xza : xoa) ==
                                                            // Changed : X.get_allocator());
                    // Changed : ASSERTV(SPEC1, SPEC2, (isPropagate ? xoa : xza) ==
                                                            // Changed : Z.get_allocator());
                    ASSERTV(SPEC1, SPEC2, (isPropagate ? &za : &oa) ==
                                                            X.get_allocator());
                    // Changed : ASSERTV(SPEC1, SPEC2, (isPropagate ? &oa : &za) ==
                                                            // Changed : Z.get_allocator());
                    ASSERTV(SPEC1, SPEC2, (isPropagate ? &oa : &oa) ==
                                                            Z.get_allocator());



                    if (isPropagate || 0 == LENGTH2) {
                        // Changed : ASSERTV(SPEC1, SPEC2, oam.isTotalSame());
                        // ^ Not true in the new model.
                    }
                    else {
                        // Changed : ASSERT(!PLAT_EXC || 1 < numPasses);
                        // Changed : ASSERTV(SPEC1, SPEC2, oam.isTotalUp());
                    }

                    if (isPropagate || 0 == LENGTH1) {
                        // Changed : ASSERTV(SPEC1, SPEC2, zam.isTotalSame());
                        // ^ Not true in the new model
                    }
                    else {
                        // Changed : ASSERTV(SPEC1, SPEC2, zam.isTotalUp());
                    }
                } BSLMA_TESTALLOCATOR_EXCEPTION_TEST_END
            }
        }
              ...
              "Invoke free 'swap' function in a context where ADL is used.\n");
    {
        ...
        // Changed : Obj mX(xoa);  const Obj& X  = gg(&mX,  "DD");
        // Changed : Obj mXX(xsa); const Obj& XX = gg(&mXX, "DD");

        // Changed : Obj mY(xoa);  const Obj& Y  = gg(&mY,  "ABC");
        // Changed : Obj mYY(xsa); const Obj& YY = gg(&mYY, "ABC");

        Obj& mX = *new(oa) Obj();
        bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2745(&mX, &oa);
        const Obj& X  = gg(&mX, "DD");

        Obj& mXX = *new(sa) Obj();
        bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2749(&mXX, &sa);
        const Obj& XX = gg(&mXX, "DD");

        Obj& mY = *new(oa) Obj();
        bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2753(&mY, &oa);
        const Obj& Y  = gg(&mY,  "ABC");

        Obj& mYY = *new(sa) Obj();
        bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2757(&mYY, &sa);
        const Obj& YY = gg(&mYY, "ABC");
    ...
    // Changed : ASSERTV(e_STATEFUL == s_allocCategory || 0 == da.numBlocksTotal());
    // ^ Not true in the new model!
}
                     ...
                     testCase7_select_on_container_copy_construction_dispatch()
{
    ...
    for (int ti = 0; ti < NUM_SPECS; ++ti) {
        ...
        // Changed : StdAlloc ma(&oa);

        {
            // Changed : Obj mW(ma); const Obj& W = gg(&mW, SPEC);  // control
            Obj& mW = *new(oa) Obj();
            bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2828(&mW, &oa);
            const Obj& W = gg(&mW, SPEC);  // control

            ASSERTV(ti, LENGTH == W.size());  // same lengths
            if (veryVerbose) { printf("\tControl Obj: "); P(W); }

            // Changed : Obj mX(ma); const Obj& X = gg(&mX, SPEC);
            Obj& mX = *new(oa) Obj();
            bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2837(&mX, &oa);
            ...
            ASSERTV(SPEC, W == Y);
            ASSERTV(SPEC, W == X);
            // Changed : ASSERTV(SPEC, PROPAGATE, PROPAGATE == (ma == Y.get_allocator()));
            // Changed : ASSERTV(SPEC, PROPAGATE,               ma == X.get_allocator());
...
{
    // Lester : This test case doesn't make sense in the new allocator model, so I am deleting it.
    // ------------------------------------------------------------------------
...
// Unchanged : template <class TYPE>
// Unchanged : void TestDriver<TYPE>::testCase7()
{
    // ------------------------------------------------------------------------
                        ...
                        TYPE_MOVE);
    {
        ...
        for (int ti = 0; ti < NUM_SPECS; ++ti) {
            ...
            for (int ei = 0; ei < NUM_EXTEND; ++ei) {
                ...
                // Changed : Obj  *pX = new Obj(xta);

                Obj* pX = new(ta) Obj();

                // bslma::RawDeleterProctor<Obj, bslma::Allocator> dp3037(&pX, &ta);
                ...
                {   // Testing concern 1.
                    ...
                    ASSERTV(SPEC, N, W == Y0);
                    ASSERTV(SPEC, N, W == X);
                    // Changed : ASSERTV(SPEC, N, Y0.get_allocator() ==
                                           // Changed : bslma::Default::defaultAllocator());
                ...
                {   // Testing concern 5 with test allocator.
                    ...
                    // Changed : Obj Y11(X, xta);
                    Obj& Y11 = *new(ta) Obj(X);
                    bslma::RawDeleterProctor<Obj, bslma::Allocator> dp2745(&Y11, &ta);
                    ...
                    if (LENGTH == 0) {
                        // Changed : ASSERTV(SPEC, N, BB + 0 == AA);
                        // Changed : ASSERTV(SPEC, N, B  + 0 == A );

                        ASSERTV(SPEC, N, BB + 1 == AA);
                        ASSERTV(SPEC, N, B  + 1 == A );
                    }
                    else {
                        const  bsls::Types::Int64 TYPE_ALLOCS =
                                                         TYPE_ALLOC * X.size();
                        // Changed : ASSERTV(SPEC, N, BB + 1 + TYPE_ALLOCS == AA);
                        // Changed : ASSERTV(SPEC, N, B  + 1 + TYPE_ALLOCS == A );

                        // Changed : ASSERTV(SPEC, N, BB + 2 + TYPE_ALLOCS == AA);
                        // Changed : ASSERTV(SPEC, N, B  + 2 + TYPE_ALLOCS == A );
                    }
                    ...
                    for (int i = 1; i < N+1; ++i) {
                        ...
                        // Changed : const bsls::Types::Int64 TYPE_ALLOC_MOVES =
                                        // Changed : TYPE_ALLOC * (2 + oldSize * TYPE_MOVE);

                        if (LENGTH == 0 && i == 1) {
                            // Changed : ASSERTV(SPEC, N, CC, DD, TYPE_ALLOC_MOVES,
                                    // Changed :CC + 1 + TYPE_ALLOC_MOVES == DD);
                            // Changed :ASSERTV(SPEC, N, i, C + 1 + TYPE_ALLOC == D);
                            // Changed :ASSERTV(SPEC, N, i, Y11.capacity() == 1);
                            // Changed : ASSERTV(SPEC, N, CC, DD, TYPE_ALLOC_MOVES,
                                    // Changed : CC + 2 + TYPE_ALLOC_MOVES == DD);
                            // Changed : ASSERTV(SPEC, N, i, C + 2 + TYPE_ALLOC == D);
                            // Changed : ASSERTV(SPEC, N, i, Y11.capacity() == 1);
                        }
                        else if (remSlots == 0) {
                            // Changed : ASSERTV(SPEC, CC, DD, TYPE_ALLOC_MOVES,
                                    // Changed : CC + 1 + TYPE_ALLOC_MOVES == DD);
                            // Changed : ASSERTV(SPEC, N, i, C + 0 + TYPE_ALLOC ==  D);

                            // Changed : ASSERTV(SPEC, CC, DD, TYPE_ALLOC_MOVES,
                                    // Changed : CC + 2 + TYPE_ALLOC_MOVES == DD);
                            // Changed : ASSERTV(SPEC, N, i, C + 1 + TYPE_ALLOC ==  D);

                            // Changed : ASSERTV(SPEC, N, i, Y11.capacity() == 2 * oldCap);

                        }
                        else {
                            // Changed : ASSERTV(SPEC, N, i, CC, DD,
                                    // Changed : CC + 0 + 2 * TYPE_ALLOC == DD);
                            // Changed : ASSERTV(SPEC, N, i, C + 0 + TYPE_ALLOC ==  D);
                            // Changed : ASSERTV(SPEC, N, i, Y11.capacity() == oldCap);
                ...
                {   // Exception checking.
                    ...
                    BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(ta) {
                        // Changed : const Obj Y2(X, xta);
                        Obj& Y2 = *new(ta) Obj(X);
                        bslma::RawDeleterProctor<Obj, bslma::Allocator> dp3226(&Y2, &ta);
                    ...
                    if (LENGTH == 0) {
                        ...
                        // Changed : const bsls::Types::Int64 TYPE_ALLOCS = TYPE_ALLOC *
                                          // Changed : (LENGTH + LENGTH * (1 + LENGTH) / 2);
                        // Changed : ASSERTV(SPEC, N, BB, AA, BB + 1 + TYPE_ALLOCS == AA);
...
// Unchanged : template <class TYPE>
// Unchanged : void TestDriver<TYPE>::testCase6()
{
    // ------------------------------------------------------------------------
    ...
    // Changed : ALLOC                xta1(&ta1);
    // Changed : ALLOC                xta2(&ta2);

    // Changed : ALLOC ALLOCATOR[] = {
        // Changed : xta1,
        // Changed : xta2
    // Changed : };
    ...
    if (verbose) printf("\tCompare each pair without perturbation.\n");
    {
        ...
        for (int si = 0; si < NUM_SPECS; ++si) {
            for (int ai = 0; ai < NUM_ALLOCATOR; ++ai) {
                ...
                // Changed : Obj mU(ALLOCATOR[ai]); const Obj& U = gg(&mU, U_SPEC);
                Obj& mU = *new(*ALLOCATOR[ai]) Obj();
                bslma::RawDeleterProctor<Obj, bslma::Allocator> dp3380(&mU, ALLOCATOR[ai]);
                const Obj& U = gg(&mU, U_SPEC);
                ...
                for (int sj = 0; sj < NUM_SPECS; ++sj) {
                    for (int aj = 0; aj < NUM_ALLOCATOR; ++aj) {
                        ...
                        // Changed : Obj               mV(ALLOCATOR[aj]);
                        // Changed : const Obj&        V = gg(&mV, V_SPEC);
                        Obj& mV = *new(*ALLOCATOR[aj]) Obj();
                        bslma::RawDeleterProctor<Obj, bslma::Allocator> dp3406(&mV, ALLOCATOR[aj]);
                        const Obj&        V = gg(&mV, V_SPEC);
    ...
    if (verbose) printf("\tCompare each pair after perturbing.\n");
    {
        ...
        for (int si = 0;  si < NUM_SPECS; ++si) {
            for (int ai = 0; ai < NUM_ALLOCATOR; ++ai) {
                ...
                // Changed : Obj        mU(ALLOCATOR[ai]);
                // Changed : const Obj& U = gg(&mU, U_SPEC);
                Obj& mU = *new(*ALLOCATOR[ai]) Obj();
                bslma::RawDeleterProctor<Obj, bslma::Allocator> dp3406(&mU, ALLOCATOR[ai]);
                const Obj& U = gg(&mU, U_SPEC);
                ...
                for (int sj = 0; sj < NUM_SPECS; ++sj) {
                    for (int aj = 0; aj < NUM_ALLOCATOR; ++aj) {
                        ...
                        for (int e = 0; e < NUM_EXTEND; ++e) {
                            ...
                            // Changed : const Obj&        V = gg(&mV, V_SPEC);
                            Obj& mV = *new(*ALLOCATOR[aj]) Obj();
                            bslma::RawDeleterProctor<Obj, bslma::Allocator> dp3468(&mV, ALLOCATOR[aj]);
                            const Obj&        V = gg(&mV, V_SPEC);
...
// Unchanged : template <class TYPE>
// Unchanged : void TestDriver<TYPE>::testCase4a()
{
    // ------------------------------------------------------------------------
    // BASIC ACCESSORS
        ...
        for (int ti = 0; ti < NUM_DATA; ++ti) {
            ...
            for (char cfg = 'a'; cfg <= 'd'; ++cfg) {
                ...
                switch (CONFIG) {
                  ...
                  case 'b': {
                      // Changed : objPtr = new (fa) Obj((bslma::Allocator *)0);
                      objPtr = new (fa) Obj();
                      objAllocatorPtr = &da;
                  } break;
                  case 'c': {
                      // Changed : objPtr = new (fa) Obj(&sa1);
                      objPtr = new (fa) Obj();
                      objAllocatorPtr = &sa1;
                  } break;
                  case 'd': {
                      // Changed : objPtr = new (fa) Obj(&sa2);
                      objPtr = new (fa) Obj();
                      objAllocatorPtr = &sa2;
                  } break;
                  default: {
                ...
                // Changed : ASSERT(&oa == X.get_allocator());
                ...
                {
                    // Changed : Obj mY(&oa); const Obj& Y = mY;
                    // Changed : Obj mZ(&oa); const Obj& Z = mZ;
                    Obj& mY = *new(oa) Obj();
                    bslma::RawDeleterProctor<Obj, bslma::Allocator> dp3673(&mY, &oa);
                    const Obj& Y = mY;

                    Obj& mZ = *new(oa) Obj();
                    bslma::RawDeleterProctor<Obj, bslma::Allocator> dp3676(&mZ, &oa);
                    const Obj& Z = mZ;
                         ...
                         " by at() when pos >= size().\t");
    {
        for (int ti = 0; ti < NUM_DATA; ++ti) {
            ...
            for (char cfg = 'a'; cfg <= 'd'; ++cfg) {
                ...
                // Changed : ASSERTV(LINE, CONFIG, noa.numBlocksTotal(),
                        // Changed : 0 == noa.numBlocksTotal());
...
// Unchanged : template <class TYPE>
// Unchanged : void TestDriver<TYPE>::testCase4()
{
    // ------------------------------------------------------------------------
    // BASIC ACCESSORS
    //   Ensure each basic accessor:
        ...
        for (int ti = 0; ti < NUM_DATA; ++ti) {
            ...
            for (char cfg = 'a'; cfg <= 'd'; ++cfg) {
                ...
                switch (CONFIG) {
                  ...
                  case 'b': {
                      // Changed : objPtr = new (fa) Obj((bslma::TestAllocator *)0);
                      objPtr = new (fa) Obj();
                      objAllocatorPtr = &da;
                  } break;
                  case 'c': {
                ...
                // Changed : ASSERTV(LINE, SPEC, CONFIG, &oa == X.get_allocator());
...

                {
                    ...
                    // Changed : Obj        mY(&oa);
                    Obj& mY = *new(oa) Obj();
                    bslma::RawDeleterProctor<Obj, bslma::Allocator> dp4060(&mY, &oa);
                    const Obj& Y = gg(&mY, SPEC);
...
// Unchanged : template <class TYPE>
// Unchanged : void TestDriver<TYPE>::testCase3()
{
    // ------------------------------------------------------------------------
    ...
    if (verbose) printf("\tTesting generator on valid specs.\n");
    {
        ...
        for (int ti = 0; ti < NUM_DATA ; ++ti) {
            ...
            // Changed : Obj        mX(xoa);
            Obj& mX = *new(oa) Obj();
            bslma::RawDeleterProctor<Obj, bslma::Allocator> dp4191(&mX, &oa);
            ...
            // Changed : Obj        mY(xoa);
            Obj& mY = *new(oa) Obj();
            bslma::RawDeleterProctor<Obj, bslma::Allocator> dp4200(&mY, &oa);
    ...
    if (verbose) printf("\tTesting generator on invalid specs.\n");
    {
        ...
        for (int ti = 0; ti < NUM_DATA ; ++ti) {
            ...
            // Changed : Obj mX(xoa);
            Obj& mX = *new(oa) Obj();
            bslma::RawDeleterProctor<Obj, bslma::Allocator> dp4200(&mX, &oa);
...
// Unchanged : template <class TYPE>
// Unchanged : void TestDriver<TYPE>::testCase2a()
{
    // ------------------------------------------------------------------------
    ...
    if (verbose) printf("\t\tPassing in an allocator.\n");
    {
        ...
        // Changed : const Obj X(xta);
        const Obj& X = *new(ta) Obj();
        bslma::RawDeleterProctor<const Obj, bslma::Allocator> dp4411(&X, &ta);
        ...
        // Changed : ASSERT(AA + 0 == BB);
        // Changed : ASSERT(A  + 0 == B);
    ...
    if (verbose) printf("\t\tIn place using a buffer allocator.\n");
    {
        ...
        // Changed : Obj mX(xoa);
        const Obj& mX = *new(oa) Obj();
        bslma::RawDeleterProctor<const Obj, bslma::Allocator> dp4432(&mX, &oa);

        // Changed : ASSERT(0 == da.numBytesInUse());
        // Changed : ASSERT(0 == oa.numBytesInUse());
    }
        ...
        printf("\tTesting 'push_back' (bootstrap) with allocator.\n");
    {
        ...
        for (size_t li = 0; li < NUM_TRIALS; ++li) {
            ...
            // Changed : Obj mX(xta);  const Obj& X = mX;
            Obj& mX = *new(ta) Obj();
            bslma::RawDeleterProctor<Obj, bslma::Allocator> dp4490(&mX, &ta);
            const Obj& X = mX;
            ...
            if (li == 0) {
                // Changed : ASSERTV(li, BB + 1 + TYPE_ALLOC == AA);
                // Changed : ASSERTV(li, B  + 1 + TYPE_ALLOC == A);
            }
            else if ((li & (li - 1)) == 0) {
                const bsls::Types::Int64 TYPE_ALLOC_MOVES =
                                            TYPE_ALLOC * (1 + li * TYPE_MOVE);
                // Changed : ASSERTV(li, BB + 1 + TYPE_ALLOC_MOVES == AA);
                // Changed : ASSERTV(li, B  + 0 + TYPE_ALLOC       == A);
            }
            else {
                // Changed : ASSERTV(li, BB + 0 + TYPE_ALLOC == AA);
                // Changed : ASSERTV(li, B  + 0 + TYPE_ALLOC == A);
            }
    ...
    if (verbose) printf("\tTesting 'clear' with allocator.\n");
    {
        ...
        for (size_t li = 0; li < NUM_TRIALS; ++li) {
            ...
            // Changed : Obj mX(xta);  const Obj& X = mX;
            Obj& mX = *new(ta) Obj();
            bslma::RawDeleterProctor<Obj, bslma::Allocator> dp4602(&mX, &ta);
            const Obj& X = mX;
            ...
            // Changed : ASSERTV(li, BB                           == AA);
            // Changed : ASSERTV(li, BB    + (int)li * TYPE_ALLOC == CC);

            // Changed : ASSERTV(li, B - 0 - (int)li * TYPE_ALLOC == A);
            // Changed : ASSERTV(li, B - 0                        == C);
        }
    }
    ...
    if (verbose) printf("\t\tWith 'push_back' only\n");
    {
        ...
        for (size_t li = 0; li < NUM_TRIALS; ++li) { // i is the length
          ...
          BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(ta) {
            ...
            // Changed : Obj mX(xta);  const Obj& X = mX;                              // 1.
            Obj& mX = *new(ta) Obj();
            bslma::RawDeleterProctor<Obj, bslma::Allocator> dp4602(&mX, &ta);
            const Obj& X = mX;

            for (size_t i = 0; i < li; ++i) {                             // 2.
                // Changed : ExceptionProctor<Obj, ALLOC> proctor(&mX, X, L_);
                // Changed : ExceptionProctor<Obj> proctor(&mX, X, L_);
...
// Unchanged : template <class TYPE>
// Unchanged : void TestDriver<TYPE>::testCase2()
{
    // ------------------------------------------------------------------------
    // TESTING PRIMARY MANIPULATORS (BOOTSTRAP)
    //   The basic concern is that the default constructor, the destructor,
    ...
    for (size_t ti = 0; ti < MAX_LENGTH; ++ti) {
        ...
        for (char cfg = 'a'; cfg <= 'c'; ++cfg) {
            ...
            switch (CONFIG) {
              ...
              case 'b': {
                  // Changed : objPtr = new (fa) Obj(ALLOC(0));
                  objPtr = new (fa) Obj();
                  objAllocatorPtr = &da;
              } break;
              case 'c': {
                  // Changed : objPtr = new (fa) Obj(xsa);
                  objPtr = new (fa) Obj();
                  objAllocatorPtr = &sa;
              } break;
              default: {
            ...
            // Changed : ASSERTV(LENGTH, CONFIG, &oa == X.get_allocator());
            ...
            // Changed : ALLOC                xscratch(&scratch);
            ...
            // Changed : if (veryVerbose) printf("\t\tTesting 'clear'.\n");
            {
                ...
                // Changed : ASSERTV(LENGTH, CONFIG, B, A,
                        // Changed : B - (int)LENGTH * TYPE_ALLOC == A);
            ...
            if (veryVerbose) printf("Hey hey hey, Lester here!\n");
            fa.deleteObject(objPtr);
            if (veryVerbose) printf("BITCONNECT!!! \n");
...
// Unchanged : template <class TYPE>
// Unchanged : void TestDriver<TYPE>::testCase1()
{
    // ------------------------------------------------------------------------
    // BREATHING TEST
    ...
    // Changed : Obj mX1(&ta);  const Obj& X1 = mX1;
    Obj& mX1 = *new(ta) Obj();
    bslma::RawDeleterProctor<Obj, bslma::Allocator> dp5072(&mX1, &ta);
    const Obj& X1 = mX1;
    ...
    if (verbose) printf("\n 2) Create a second object x2 (copy from x1)."
                         "\t\t{ x1: x2: }\n");
    // Changed : Obj mX2(X1, &ta);  const Obj& X2 = mX2;
    Obj& mX2 = *new(ta) Obj();
    bslma::RawDeleterProctor<Obj, bslma::Allocator> dp5098(&mX2, &ta);
    const Obj& X2 = mX2;
    ...
    // Changed : Obj mX3(&ta);  const Obj& X3 = mX3;
    Obj& mX3 = *new(ta) Obj();
    bslma::RawDeleterProctor<Obj, bslma::Allocator> dp5195(&mX3, &ta);
    const Obj& X3 = mX3;
    ...
    // Changed : Obj mX4(X2, &ta);  const Obj& X4 = mX4;
    Obj& mX4 = *new(ta) Obj(X2);
    bslma::RawDeleterProctor<Obj, bslma::Allocator> dp5216(&mX4, &ta);
    const Obj& X4 = mX4;
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 36: {
...

        {
            ...
            // Changed : MyMatrix<int> m1(1, 1, &oa);
            MyMatrix<int>& m1 = *new(oa) MyMatrix<int>(1, 1);
            bslma::RawDeleterProctor<MyMatrix<int>, bslma::Allocator> dp5357(&m1, &oa);
      ...
      case 1: {
        ...
        // Changed : ASSERT(  (bslmf::IsBitwiseMoveable<vector<char> >::value));
        // Changed : ASSERT(
            // Changed : (bslmf::IsBitwiseMoveable<vector<bsltf::AllocTestType> >::value));
        // Changed : ASSERT(  (bslmf::IsBitwiseMoveable<vector<Vector_Imp<int> > >::value));
#endif
